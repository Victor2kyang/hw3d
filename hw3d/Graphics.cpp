...
//用于加载shader编译后的cso二进制文件
#pragma comment(lib,"D3DCompiler.lib")

// graphics exception checking/throwing macros (some with dxgi infos)
//这些宏大多用来包裹特定返回值的com调用，以获取/弹出调试信息
...
void Graphics::DrawTestTriangle(float angle)
{
	...
	//vertices[0].color.r = 155;
	//vertices[0].color.g = 155;
	...
	//begin  ---  create vertex shader
	wrl::ComPtr<ID3D11VertexShader> pVertexShader;
	//wrl::ComPtr<ID3DBlob> pBlob;
	//D3DReadFileToBlob(L"VertexShader.cso",&pBlob);//load Shader 必须适用wild string
	GFX_THROW_INFO(D3DReadFileToBlob(L"VertexShader.cso", &pBlob));
	//从pBlob中的信息创建真正的shader
	...
	//bind render target 绑定渲染目标
	//pContext->OMSetRenderTargets(1u,&pTarget,nullptr );
	pContext->OMSetRenderTargets(1u, pTarget.GetAddressOf(), nullptr);

	//设置拓扑类型 Primitive Topologies 常见有点，线，三角形，
